...
{
	ShowError(GetMessage('ASPRO_ALLCORP2_MODULE_NOT_INSTALLED'));
	return;
}

//$this->setFramemode(false);
$arModuleOptions = CAllcorp2::GetFrontParametrsValues(SITE_ID);

if($arModuleOptions['ORDER_VIEW'] === 'Y')
{
	if($arParams['CHECK_BASKET_URL'] == 'Y' && (CAllcorp2::IsBasketPage($arModuleOptions["URL_BASKET_SECTION"]) || CAllcorp2::IsOrderPage($arModuleOptions["URL_ORDER_SECTION"])))
	{
		return;
	}
	else
	{
		...
		$arParams['PATH_TO_CATALOG'] = (strlen(trim($arModuleOptions['URL_CATALOG_SECTION'])) ? trim($arModuleOptions['URL_CATALOG_SECTION']) : '');
		...
		if(isset($_SESSION[SITE_ID][$userID]['BASKET_ITEMS']) && is_array($_SESSION[SITE_ID][$userID]['BASKET_ITEMS']) && $_SESSION[SITE_ID][$userID]['BASKET_ITEMS'])
		{
			...
			foreach($arItems as $arItem)
			{
				...
				{
					$arItem['SUMM'] = CAllcorp2::FormatSumm($arItem['PROPERTY_FILTER_PRICE_VALUE'], $arItem['QUANTITY']);
					$allSumm += floatval(str_replace(' ', '', $arItem['PROPERTY_FILTER_PRICE_VALUE'])) * $arItem['QUANTITY'];
				}
				$arResult['ITEMS'][$arItem['ID']] = $arItem;
			}
			
			$arResult['ALL_SUM'] = CAllcorp2::FormatSumm($allSumm, 1);
