									...
									<?if(is_array($arResult["QUESTIONS"])):?>
										<?foreach( $arResult["QUESTIONS"] as $FIELD_SID => $arQuestion ){
											...
											if( $arQuestion['STRUCTURE'][0]['FIELD_TYPE'] == 'hidden' ){
												...
												<div class="row<?=($hidden ? ' hidden' : '');?> <?=(strpos($FIELD_SID, 'HIDDEN') !== false ? 'hidden' : '');?>" data-SID="<?=$FIELD_SID?>">
								...
								<div class="col-md-12 col-sm-12" style="margin-top: 26px;">
									<?if($arParams["SHOW_LICENCE"] == "Y"):?>
										<div class="licence_block bx_filter">
											<input type="checkbox" id="licenses" <?=(COption::GetOptionString("aspro.allcorp2", "LICENCE_CHECKED", "N") == "Y" ? "checked" : "");?> name="licenses" required value="Y">
											<label for="licenses">
...
<script>
	$(document).ready(function(){
		...
		if(arAllcorp2Options['THEME']['PHONE_MASK'].length){
			...
			$('.order.form form[name="<?=$arResult["IBLOCK_CODE"]?>"] input.phone').inputmask("mask", { "mask": arAllcorp2Options['THEME']['PHONE_MASK'], 'showMaskOnHover': false });
