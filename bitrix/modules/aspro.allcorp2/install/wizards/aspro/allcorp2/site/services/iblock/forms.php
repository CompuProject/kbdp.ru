...
$arFormsShortCODE = array("example", "order_project", "order_product", "order_services", "resume", "question", "callback", "order_page", "order_study", "director");
foreach($arFormsShortCODE as $iblockShortCODE){
	...
	$iblockTYPE = "aspro_allcorp2_form";
	...
	if(WIZARD_INSTALL_DEMO_DATA){
		...
		elseif($iblockShortCODE == "callback")
		{
			// edit form user oprions
			CUserOptions::SetOption("form", "form_element_".$iblockID, array(
				"tabs" => 'edit1--#--'.GetMessage("WZD_OPTION_106").'--,--ACTIVE--#--'.GetMessage("WZD_OPTION_2").'--,--NAME--#--'.GetMessage("WZD_OPTION_8").'--,--XML_ID--#--'.GetMessage("WZD_OPTION_12").'--,--IBLOCK_ELEMENT_PROP_VALUE--#--'.GetMessage("WZD_OPTION_16").'--,--PROPERTY_'.$arProperty["FIO"].'--#--'.GetMessage("WZD_OPTION_251").'--,--PROPERTY_'.$arProperty["PHONE"].'--#--'.GetMessage("WZD_OPTION_118").'--;--;--',
			));

			// list user options
			CUserOptions::SetOption("list", "tbl_iblock_list_".md5($iblockTYPE.".".$iblockID), array(
				"columns" => 'PROPERTY_'.$arProperty["FIO"].',PROPERTY_'.$arProperty["PHONE"].',PROPERTY_'.$arProperty["EMAIL"].',TIMESTAMP_X,ID', "by" => "date_active_from", "order" => "desc", "page_size" => "20", 
			));
		}
		...
		elseif($iblockShortCODE == "order_page")
		{
			// edit form user oprions
			CUserOptions::SetOption("form", "form_element_".$iblockID, array(
