...
if($arResult['ITEMS'])
{
	$arAllSections = CAllcorp2::GetSections($arResult['ITEMS'], $arParams);

	$bHasImg = false;
	$arHideProps = array('METRO', 'SCHEDULE', 'EMAIL', 'PHONE', 'PAY_TYPE');
	foreach($arResult['ITEMS'] as $key_item => $arItem)
	{
		CAllcorp2::getFieldImageData($arItem, array('PREVIEW_PICTURE'));
		...
		{
			foreach($arItem['DISPLAY_PROPERTIES'] as $key_prop => $arProperty)
			{
				if(in_array($key_prop, $arHideProps))
					unset($arResult['ITEMS'][$key_item]['DISPLAY_PROPERTIES'][$key_prop]);
			}
		}
	}
