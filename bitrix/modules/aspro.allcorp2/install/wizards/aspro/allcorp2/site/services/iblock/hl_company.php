...
unset($_SESSION["ALLCORP2_HBLOCK_COMPANY_ID"]);

$dbHblock = HL\HighloadBlockTable::getList(array("filter" => array("NAME" => "AsproAllcorp2CompanyReference")));
if(!$arBlock = $dbHblock->Fetch()){
	$data = array('NAME' => 'AsproAllcorp2CompanyReference', 'TABLE_NAME' => 'b_hlbd_svoystvakompanii');
	$result = HL\HighloadBlockTable::add($data);
	$HL_ID = $result->getId();
	$_SESSION["ALLCORP2_HBLOCK_COMPANY_ID"] = $HL_ID;

	CWizardUtil::ReplaceMacrosRecursive(WIZARD_SITE_PATH, Array("HIBLOCK_COMPANY_ID" => $HL_ID));
	CWizardUtil::ReplaceMacrosRecursive($bitrixTemplateDir, Array("HIBLOCK_COMPANY_ID" => $HL_ID));
	...
	if($HL_ID){
		...
		$arUserFields = array (
			...
			array (
				...
				'USER_TYPE_ID' => 'string',
				'XML_ID' => 'UF_PROFIT_FULL_DESCRIPTION',
				'SORT' => '600',
				'MULTIPLE' => 'N',
				'MANDATORY' => 'N',
				'SHOW_FILTER' => 'N',
			...
			array (
				'ENTITY_ID' => 'HLBLOCK_'.$HL_ID,
				'FIELD_NAME' => 'UF_AFTER_TEXT',
				'USER_TYPE_ID' => 'string',
				'XML_ID' => 'UF_PROFIT_AFTER_TEXT',
				'SORT' => '600',
				'MULTIPLE' => 'N',
				'MANDATORY' => 'N',
				'SHOW_FILTER' => 'N',
...
{
	CWizardUtil::ReplaceMacrosRecursive(WIZARD_SITE_PATH, Array("HIBLOCK_COMPANY_ID" => $arBlock["ID"]));
	CWizardUtil::ReplaceMacrosRecursive($bitrixTemplateDir, Array("HIBLOCK_COMPANY_ID" => $arBlock["ID"]));
}
?>