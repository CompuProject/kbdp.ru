...
if(!class_exists('CAllcorp2Regionality'))
{
	class CAllcorp2Regionality{
		...
		public static function checkUseRegionality(){
			if(\Bitrix\Main\Loader::includeModule(ALLCORP2_MODULE_ID))
			{
				return CAllcorp2::GetFrontParametrValue('USE_REGIONALITY');
			}
			return 'N';
		}
		...
		public static function getRegionIBlockID(){
			...
			{
				if(isset(CCache::$arIBlocks[SITE_ID]['aspro_allcorp2_regionality']['aspro_allcorp2_regions'][0]) && CCache::$arIBlocks[SITE_ID]['aspro_allcorp2_regionality']['aspro_allcorp2_regions'][0])
				{
					$iRegionIBlockID = CCache::$arIBlocks[SITE_ID]['aspro_allcorp2_regionality']['aspro_allcorp2_regions'][0];
				}
				else
				{
					return;
		...
		public static function replaceSeoMarks(){
			...
			if($arRegion && $page_title)
			{
				...
				if(!CAllcorp2::IsMainPage())
				{
					$bShowSiteName = (\Bitrix\Main\Config\Option::get(ALLCORP2_MODULE_ID, "HIDE_SITE_NAME_TITLE", "N") == "N");
		...
		public static function getRegions(){
			...
			if($arRegions === NULL)
			{
				...
				if($iRegionIBlockID = self::getRegionIBlockID())
				{
					...
					$cache_id = 'aspro_allcorp2_regions'.$iRegionIBlockID;
					...
					{
						// get all items
						$arMainProps = array('DEFAULT', 'DOMAINS', 'MAIN_DOMAIN', 'FAVORIT_LOCATION', 'PHONES', 'PRICES_LINK', 'LOCATION_LINK', 'STORES_LINK', 'REGION_NAME_DECLINE_RP', 'REGION_NAME_DECLINE_PP', 'REGION_NAME_DECLINE_TP', 'SORT_REGION_PRICE', 'ADDRESS', 'EMAIL', 'SHCEDULE');
						...
						foreach(GetModuleEvents(ALLCORP2_MODULE_ID, 'OnAsproRegionalityAddSelectFieldsAndProps', true) as $arEvent) // event for add to select in region getlist elements
							ExecuteModuleEventEx($arEvent, array(&$arSelect));

						// $arItems = CCache::CIBLockElement_GetList(array('SORT' => 'ASC', 'NAME' => 'ASC', 'CACHE' => array('TAG' => CCache::GetIBlockCacheTag($iRegionIBlockID), 'GROUP' => 'ID')), $arFilter, false, false, $arSelect);
						...
						foreach(GetModuleEvents(ALLCORP2_MODULE_ID, 'OnAsproRegionalityGetElements', true) as $arEvent) // event for manipulation with region elements
