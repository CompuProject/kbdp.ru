...
{
	$arResult['REGIONS'] = CAllcorp2Regionality::getRegions();
	$arResult['CURRENT_REGION'] = CAllcorp2Regionality::getCurrentRegion();
	$arResult['REAL_REGION'] = CAllcorp2Regionality::getRealRegionByIP();
	...
	if($arResult['REGIONS'])
	{
		...
		if($arResult['POPUP'])
		{
			...
			if($arSectionsID)
			{
				$arSections = CCache::CIBlockSection_GetList(array('SORT' => 'ASC', 'NAME' => 'ASC', 'CACHE' => array('GROUP' => 'ID', 'TAG' => CCache::GetIBlockCacheTag(CCache::$arIBlocks[SITE_ID]['aspro_allcorp2_regionality']['aspro_allcorp2_regions'][0]))), array('GLOBAL_ACTIVE' => 'Y', 'ID' => $arSectionsID, 'IBLOCK_ID' => CCache::$arIBlocks[SITE_ID]['aspro_allcorp2_regionality']['aspro_allcorp2_regions'][0]), false, array('ID', 'IBLOCK_ID', 'NAME', 'LEFT_MARGIN', 'RIGHT_MARGIN', 'DEPTH_LEVEL'));
				if($arSections)
				{
					$arSections2 = array();
					foreach($arSections as $key => $arSection)
					{
						...
						if($arSection['DEPTH_LEVEL'] > 1)
						{
							...
							$arTmpSection = CCache::CIBlockSection_GetList(array('CACHE' => array('MULTI' => 'N', 'TAG' => CCache::GetIBlockCacheTag(CCache::$arIBlocks[SITE_ID]['aspro_allcorp2_regionality']['aspro_allcorp2_regions'][0]))), array('GLOBAL_ACTIVE' => 'Y', '<=LEFT_BORDER' => $arSection['LEFT_MARGIN'], '>=RIGHT_BORDER' => $arSection['RIGHT_MARGIN'], 'DEPTH_LEVEL' => 1, 'IBLOCK_ID' => CCache::$arIBlocks[SITE_ID]['aspro_allcorp2_regionality']['aspro_allcorp2_regions'][0]), false, array('ID', 'IBLOCK_ID', 'NAME', 'SORT'));
