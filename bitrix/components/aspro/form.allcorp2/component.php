...
{
	$arThemes = CAllcorp2::GetFrontParametrsValues(SITE_ID);
		...
		<?if( CModule::IncludeModule("iblock") ){
			...
			if( !is_array( $arMess ) ){
				...
				$arMess["ID"] = $em->Add( array(
					...
					"EMAIL_TO" => ($arResult["IBLOCK_CODE"] == "aspro_allcorp2_callstaff" ? "#STAFF_EMAIL_HIDDEN#" : "#EMAIL#"),
					"BCC" => "",
					...
					switch( $arQuestion["FIELD_TYPE"] ){
						...
						case "checkbox":
							...
							while( $arValue = $rsValue->Fetch() ){
								...
								$html .= '<div><input class="'.$required.'" id="'.$arValue["ID"].'" name="'.$arQuestion["CODE"].($arQuestion["MULTIPLE"] == "Y" ? '[]' : '').'" type="checkbox" value="'.$arValue["ID"].'" '.$checked.' />';
								if( $count == 1 ){
									$html .= '<label for="'.$arValue["ID"].'">'.$arQuestion["NAME"].'</label>';
								}else{
									$html .= '<label for="'.$arValue["ID"].'">'.$arValue["VALUE"].'</label></div>';
								}
								$arResult["QUESTIONS"][$FIELD_CODE]["ENUMS"][$arValue["ID"]] = $arValue["VALUE"];
							}
							break;
					...
					if( count( $arResult["FORM_ERRORS"] ) <= 0 ){
							...
							if( $RESULT_ID = $el->Add( $arFields ) ){
								...
								if(strlen($arEventFields["EMAIL"])){
									if(isset($arEventFields['ORDER_LIST']) && strpos('aspro_allcorp2_order_page', $arResult['IBLOCK_CODE']) !== false){
