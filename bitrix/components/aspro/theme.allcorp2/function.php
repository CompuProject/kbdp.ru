	...
	<?elseif($arOption['TYPE'] == 'selectbox' || $arOption['TYPE'] == 'multiselectbox'):?>
		...
		<?else:?>
			...
			<?foreach($arOption['LIST'] as $variantCode => $arVariant):
				...
				<?endif;?>
				<a href="javascript:;" <?=($isPreview && (isset($arOption['PREVIEW']['SCROLL_BLOCK']) && $arOption['PREVIEW']['SCROLL_BLOCK']) ? "data-option-type='".$arOption['PREVIEW']['SCROLL_BLOCK']."'" : "");?> <?=($isPreview && isset($arOption['PREVIEW']['URL']) ? "data-option-url='".SITE_DIR.$arOption['PREVIEW']['URL']."'" : "");?> data-option-id="<?=$optionCode?>" data-option-value="<?=$variantCode?>" class="link-item animation-boxs <?=(isset($arVariant['POSITION_BLOCK']) && $arVariant['POSITION_BLOCK'] ? $arVariant['POSITION_BLOCK'] : '');?> <?=(isset($arVariant['DEPEND']) ? (in_array($arParentOption[$arVariant['DEPEND']['TYPE']]['VALUE'], $arVariant['DEPEND']['VALUE']) ? '' : 'disabled') : '');?> <?=$arVariant['CURRENT'] == 'Y' ? 'current' : ''?>">
